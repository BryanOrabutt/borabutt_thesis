...
%have to protect and create paragraph box in order to have multiline and centered title
...
\advisor{George L. Engel, D.Sc}
\secondreader{Bradley Noble, D.Sc} %% or \secondreader{Dr.}{Karl Gauss}
\thirdreader{Timothy York, PhD}
...
% ABSTRACT
% ^^^^^^^^^^^^^^^^^^^^^
%use phantom sections in front matter to have PDF anchor's created since chapter numbering is turned off at this point
\phantomsection
\begin{abstract}
...
\thispagestyle{empty} %remove page number from first page (required by grad school)
...
%         framexbottommargin=4pt,
 ...
 \lstinputlisting{./code/example.vcd}
...
\lstinputlisting{./code/example.pwl}
...
\nocite{*} %incude all refences in bibliography, not just those cited in text
...
%Appendix restarts chapter counter causing PDF anchors to point to chapter 1.
%Resetting counter and changing chapters to be alphabetized (i.e. Chapter A, B, C etc.) fixes pdf anchors
